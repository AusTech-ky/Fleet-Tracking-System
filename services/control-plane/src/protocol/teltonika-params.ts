/**
 * Teltonika configuration parameters (control-plane copy) — the "records profile" (data acquisition)
 *
 * NOTE: this file is a verbatim copy of packages/protocol-teltonika/src/params.ts.
 * The control-plane is CommonJS compiled by tsc and cannot import the native-TS
 * ESM protocol package; the catalogue is pure data + string builders with no
 * imports, so it is duplicated rather than the build system contorted. The
 * protocol package copy is canonical and carries the tests; keep them in sync
 * (test/params-sync.test.ts fails if they drift).
